...
const COMBO_WINDOW_SEC := 2.2
const COMBO_BONUS_STEP := 14
const COMBO_BONUS_CAP := 220
...
func _process(delta: float) -> void:
	...
	_tick_combo_timer(scaled_delta)
...
func _on_enemy_destroyed(enemy, by_player: bool) -> void:
	...
	if by_player:
		...
		var combo_bonus := _register_destroy_combo(enemy.is_boss)
		var display_score := int(enemy.score_value) + combo_bonus
		var popup_text := "+%d" % display_score
		if combo_count >= 2:
			popup_text = "+%d  x%02d" % [display_score, combo_count]
		var popup_color := Color(1.0, 0.84, 0.52) if enemy.is_boss else Color(1.0, 0.88, 0.66)
		if combo_count >= 8:
			popup_color = Color(1.0, 0.96, 0.56)
		var popup_scale := 1.18 if enemy.is_boss else 0.88
		popup_scale += min(combo_count, 12) * 0.012
...
func _register_destroy_combo(is_boss: bool) -> int:
	if combo_timer > 0.0:
		combo_count += 1
	else:
		combo_count = 1
	if is_boss:
		combo_count += 2
	combo_timer = COMBO_WINDOW_SEC + (0.7 if is_boss else 0.0)
	best_combo = max(best_combo, combo_count)

	var combo_bonus := 0
	if combo_count >= 3:
		combo_bonus = min((combo_count - 2) * COMBO_BONUS_STEP, COMBO_BONUS_CAP)
		if is_boss:
			combo_bonus += 280
	RunState.register_combo_progress(combo_count, combo_bonus)

	if combo_bonus > 0 and is_instance_valid(hud):
		if combo_count == 3 or combo_count == 6 or combo_count % 5 == 0 or is_boss:
			hud.set_status_hint(
				_t("连锁 x%d  +%d", "CHAIN x%d  +%d") % [combo_count, combo_bonus],
				Color(1.0, 0.9, 0.46)
			)
	return combo_bonus
