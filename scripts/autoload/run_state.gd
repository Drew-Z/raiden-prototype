...
func register_combo_progress(combo_count: int, bonus_score: int = 0) -> void:
	current_run.best_combo = max(int(current_run.get("best_combo", 0)), combo_count)
	if bonus_score <= 0:
		return
	current_run.combo_bonus_score += bonus_score
	current_run.score += bonus_score
...
func get_performance_grade() -> String:
	...
	if current_run.bombs_used <= 1:
		points += 1
	if int(current_run.get("best_combo", 0)) >= 12:
		points += 1
...
func get_score_breakdown_text() -> String:
	if not current_run.victory:
		var fail_text := _lang("战斗得分：%06d", "Battle Score: %06d") % int(current_run.final_score)
		if int(current_run.get("best_combo", 0)) >= 3:
			fail_text += _lang(
				"\n最高连锁：x%d   连锁奖励：%04d",
				"\nBest Chain: x%d   Chain Bonus: %04d"
			) % [
				int(current_run.get("best_combo", 0)),
				int(current_run.get("combo_bonus_score", 0))
			]
		return fail_text
	if is_english():
...
func get_result_tags() -> Array[String]:
	...
	if int(current_run.get("best_combo", 0)) >= 10:
		tags.append(_lang(
			"连锁 x%d",
			"CHAIN x%d"
		) % int(current_run.get("best_combo", 0)))
